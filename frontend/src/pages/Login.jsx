...
const redirectUser = (user) => {
  ...
  const loginContent = (
    <>
          ...
          <div className="relative group">
            ...
            <input
              ...
              className={`w-full pl-11 pr-4 py-3.5 bg-white dark:bg-gray-800/90 border-2 border-gray-200 dark:border-gray-700 rounded-xl focus:border-gray-400 focus:ring-0 focus:outline-none focus:shadow-[0_0_0_3px_rgba(15,23,42,0.06)] transition-all duration-200 placeholder:text-gray-400 dark:text-gray-500 shadow-sm hover:shadow-md ${
                showPasswordField && !nativeMobileMode ? 'lg:opacity-50 lg:pointer-events-none' : ''
              }`}
              placeholder={t('emailPlaceholder')}
              disabled={showPasswordField && !nativeMobileMode}
              required
            />
            {!showPasswordField && (
              <button
      ...
      <p className="flex flex-wrap items-center justify-center text-center text-gray-600 dark:text-gray-300 mt-4 sm:mt-6 text-xs sm:text-sm">
        {t('noAccount')}{' '}
        <Link to="/register" className="inline-flex min-h-11 items-center px-3 text-red-600 font-bold cursor-pointer touch-manipulation hover:text-red-700 transition-colors hover:underline hover:underline-offset-2">
          {t('createOne')}
        </Link>
      </p>
    </>
  ...
  if (isNative) {
    return (
      <MobileLoginView branding={brandingElement} footer={<LegalFooter className="relative z-10 mt-6" />}>
        {loginContent}
      </MobileLoginView>
    );
  }
