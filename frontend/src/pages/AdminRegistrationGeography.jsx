...
const AdminRegistrationGeography = () => {
  ...
  return (
    <DashboardLayout requiredRole="ADMIN">
      <DashboardHeader title={g('title')} variant="admin" />
      <div className="space-y-7 p-4 md:p-6 lg:p-8" dir="ltr">
          ...
          <div className="mt-5 space-y-4">
            {summary?.countries?.length ? summary.countries.map((country) => (
              <div key={country.countryCode}>
                ...
                <div className="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700"><div className="h-full rounded-full bg-yellow-500" style={{ width: `${Math.min(country.percentage, 100)}%` }} /></div>
              </div>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <div className="border-b border-gray-200 p-5 dark:border-gray-700"><h2 className="text-xl font-bold text-gray-900 dark:text-white">{g('userDetails')}</h2><div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            <select value={filters.role} onChange={changeFilter('role')} className="rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm dark:border-gray-600 dark:bg-gray-900"><option value="">{g('allRoles')}</option>{['ADMIN','EMPLOYER','WORKER','SUPPORT'].map((role) => <option key={role} value={role}>{g(`roles.${role}`)}</option>)}</select>
          </div></div>
...
export default AdminRegistrationGeography;
