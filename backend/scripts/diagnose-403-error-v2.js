// backend/scripts/diagnose-403-error-v2.js
...
async function diagnose403Error() {
  try {
    ...
    for (const conversationId of realConversationIds) {
      ...
      const userWithEmployerId = await User.findOne({
        $or: [
          { id: employerId },
          { _id: employerId },
          { email: employerId }
      ...
      const userWithWorkerId = await User.findOne({
        $or: [
          { id: workerId },
          { _id: workerId },
          { email: workerId }
      ...
      console.log('  User with employerId:', userWithEmployerId ? {
        id: userWithEmployerId.id,
        _id: userWithEmployerId._id,
        email: userWithEmployerId.email
      } : 'Not found');

      console.log('  User with workerId:', userWithWorkerId ? {
        id: userWithWorkerId.id,
        _id: userWithWorkerId._id,
        email: userWithWorkerId.email
      } : 'Not found');
