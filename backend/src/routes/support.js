...
// Reply to a complaint (public reply visible to user)
router.post('/complaints/:id/reply', async (req, res) => {
  try {
    ...
    await logActivity(
      supportId,
      'COMPLAINT_REPLIED',
      `Replied to complaint "${complaint.id}"`,
      complaint.userId,
      id
    );
...
router.post('/complaints/:id/notes', async (req, res) => {
  try {
    ...
    const complaint = await prisma.complaint.findUnique({
      where: { id },
      select: { id: true, internalNotes: true, userId: true },
    });
    ...
    await logActivity(
      supportId,
      'COMPLAINT_NOTED',
      `Added internal note to complaint "${id}"`,
      complaint.userId,
      id
    );
...
router.put('/complaints/:id/status', async (req, res) => {
  try {
    ...
    const validStatuses = ['NEW', 'IN_PROGRESS', 'WAITING_FOR_USER', 'ESCALATED_TO_ADMIN', 'RESOLVED', 'CLOSED'];
    ...
    await logActivity(
      ...
      `Changed complaint "${id}" status from ${complaint.status} to ${status}`,
      complaint.userId,
      id
    );
...
router.post('/complaints/:id/escalate', async (req, res) => {
  try {
    ...
    await logActivity(
      supportId,
      'COMPLAINT_ESCALATED',
      `Escalated complaint "${id}" to admin. Reason: ${reason}`,
      complaint.userId,
      id
    );
...
// Return support dashboard statistics
router.get('/stats', async (req, res) => {
  try {
    ...
    // Open complaints (NEW + IN_PROGRESS + WAITING_FOR_USER)
    ...
    const escalatedComplaints = complaintStatusStats['ESCALATED_TO_ADMIN'] || 0;
    ...
    // Users assisted today (users with support activity today)
